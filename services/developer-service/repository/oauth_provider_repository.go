...
func (r *OAuthProviderRepository) Create(ctx context.Context, appID, provider, clientID, clientSecretEncrypted string, scopes []string) (*OAuthProvider, error) {
	...
	err := r.db.QueryRow(ctx, `
		...
		RETURNING id, app_id, provider, client_id, client_secret_encrypted, scopes, enabled, created_at, updated_at
	`, appID, provider, clientID, clientSecretEncrypted, scopes).Scan(
		&op.ID, &op.AppID, &op.Provider, &op.ClientID, &op.ClientSecretEncrypted, &op.Scopes, &op.Enabled, &op.CreatedAt, &op.UpdatedAt,
	)
	if err != nil {
		return nil, err
	}
...
func (r *OAuthProviderRepository) FindByAppAndProvider(ctx context.Context, appID, provider string) (*OAuthProvider, error) {
	op := &OAuthProvider{}
	err := r.db.QueryRow(ctx, `
		SELECT id, app_id, provider, client_id, client_secret_encrypted, scopes, enabled, created_at, updated_at
...
func (r *OAuthProviderRepository) ListByAppID(ctx context.Context, appID string) ([]*OAuthProvider, error) {
	...
	for rows.Next() {
		op := &OAuthProvider{}
		err := rows.Scan(&op.ID, &op.AppID, &op.Provider, &op.ClientID, &op.ClientSecretEncrypted, &op.Scopes, &op.Enabled, &op.CreatedAt, &op.UpdatedAt)
		if err != nil {
			return nil, err
		}
