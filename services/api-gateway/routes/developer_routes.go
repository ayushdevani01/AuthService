...
func (dr *DeveloperRoutes) CreateApp(c *gin.Context) {
	...
	c.JSON(http.StatusCreated, gin.H{
		...
		"integration": gin.H{
			"publishable_key":   resp.App.AppId,
			...
			"notes": []string{
				"Use publishable_key for JWKS, hosted login, JWT aud, and x-app-id.",
...
func formatApp(a *pb.App) gin.H {
	...
	return gin.H{
		...
		"publishable_key": a.AppId,
		"audience_app_id": a.AppId,
		"identifier_usage": gin.H{
			"publishable_key": "Use for JWKS, hosted login, JWT aud, and x-app-id.",
			"public_app_id":   "Alias of publishable_key.",
		},
