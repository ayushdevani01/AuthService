...
const authOptions = {
  appId: process.env.AUTH_APP_ID || process.env.AUTH_PUBLISHABLE_KEY || 'your-publishable-key',
...
app.listen(PORT, () => {
  ...
  console.log(`   AUTH_APP_ID - Your publishable key (JWKS + JWT aud)`);
  ...
  console.log(`\nUsage: Set AUTH_APP_ID, then make requests with 'Authorization: Bearer <jwt-token>'`);
});
